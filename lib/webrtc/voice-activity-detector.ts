...
export class VoiceActivityDetector {
  ...
  private analyze(): void {
    ...
    if (average > threshold) {
      ...
      if (!this.isSpeaking && (now - this.speechStartTime) >= this.options.minSpeechDuration) {
        this.startSpeech();
      }
    } else {
      // No voice detected
      if (this.isSpeaking && (now - this.lastSpeechTime) >= this.options.silenceDuration) {
        this.endSpeech();
      }
    }
  ...
  setAIMode(enabled: boolean): void {
    ...
    console.log(`AI mode ${enabled ? 'enabled' : 'disabled'} - sensitivity adjusted`);
  }
...
export function createVoiceActivityDetector(options?: Partial<VoiceActivityOptions>): VoiceActivityDetector {
  return new VoiceActivityDetector(options);
}

// Utility function to test microphone and VAD
export async function testVoiceActivityDetection(duration: number = 5000): Promise<void> {
